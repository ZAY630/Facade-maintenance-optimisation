...
% The code is established based on cladding degradation and maintenance
% intervention. The simulation of maintenance effect is the same as the
% content in file MOP.
function R = Reliability5(z)
    ...
    % Schedule summary (preventative, corrective and replacement throughout 150 years)
    m = 0;
    n = length(nonzeros(y));
    for i = 1:n
        if y(i) == 0
    ...
    % Reference time-based replacement schedule
    x = 1:100;
    ...
    % Reliability caculation
    count = 1;
    reliability = zeros(1, 150);
    reliability_reference = zeros(1, 150);
    
    % Original (highest relaibility index) starting at 4.75, which means
    % failure probability is 10^-6.
    reliability(1) = 4.75;
    reliability_reference(1) = 4.75;
    
    % Moisture attack 
    % When degradation above 10%, internal frame is deterioration.
    for i = 2:150
        if lifespan(i) <= 0.10
    ...
    % Find final reliability index after 150 years
    R = reliability(end);
